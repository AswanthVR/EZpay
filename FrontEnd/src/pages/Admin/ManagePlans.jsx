...
const ManagePlans = () => {
    ...
    useEffect(() => {
        ...
        const fetchPlans = async () => {
            try {
                const response = await axios.get('http://localhost:8181/ezpay/api/v1/plans', {
                    headers: {
    ...
    const handleDelete = async (planId) => {
        try {
            await axios.delete(`http://localhost:8181/ezpay/api/v1/plans/${planId}`, {
                headers: {
    ...
    const handleSaveEdit = async (updatedPlan) => {
        try {
            const response = await axios.put(`http://localhost:8181/ezpay/api/v1/plans/${updatedPlan.planId}`, updatedPlan, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            });
